...
function fillSelectorButtons(dayIndex) {
  ...
  btns_charlas.forEach((btn, i) => {
    ...
    btn.querySelector(".descripcion-ponencia-btn").textContent =
      `${ch.horario} || ${ch.aula}`;
  });
}
...
function fillDescription(day_index, talk_index) {
  ...
  // Modificar parte del ponente
  const contenedorPonente = desc_ponencia.children[0];
  const elNombre = contenedorPonente.querySelector(".ponente-nombre");
  const elSocial = contenedorPonente.querySelector(".ponente-social");

  elNombre.textContent = ch.ponente;
  // Manejar la red social
  if (ch.social_network && ch.social_network.username) {
    elSocial.textContent = `@${ch.social_network.username}`;
    elSocial.href = ch.social_network.url;
    elSocial.style.display = "block"; // Mostrar si existe
  } else {
    elSocial.textContent = "";
    elSocial.style.display = "none"; // Ocultar si no hay datos
  }

  // Modificar horario y aula (el segundo hijo)
